...
//reset-request-customer.php file for Jon's Snow Removal Page
...
if(isset($_POST["reset-request-submit"])){
    ...
    //Create URL
    $url = "http://localhost/jonssnow/create-new-pw-customer.php?selector=" . $selector . "&validator=" . bin2hex($token);
    ...
    $sql = "DELETE FROM pwdresetuser WHERE pwdResetEmail=?";
    ...
    /*Step 2:  Insert new reset request from the user.*/

    //Create new sql prepared statement to insert new user request into database.
    $sql = "INSERT INTO pwdresetuser (pwdResetEmail, pwdResetSelector, pwdResetToken, pwdResetExpires) VALUES (?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($conn);
    ...
    $to = $userEmail;
    $subject = "Reset your password for Snow Day.";
    ...
    if(mail($to, $subject, $message, $headers)){   
        ...
        echo "<script type='text/javascript'> document.location.href = 'forgot-pw-customer.php?reset=success';</script>";
        
    } else{
       echo "Unable to send email. Back to <a href='login-customer.php'>login page</a>. ";
    }
    
} else{
    echo "There is nothing in the POST.  Back to <a href='login-customer.php'>login page</a>.";
