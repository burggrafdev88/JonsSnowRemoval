...
<?php
    ...
    $query="SELECT * FROM bid JOIN request ON bid.request_id = request.idRequests JOIN est_service_time ON bid.estimated_completion_time = est_service_time.id_service_time JOIN bid_status ON bid.status = bid_status.id WHERE (provider_ID = '$providerID') AND (bid.status=1)";
...
<body>
    ...
    <main>
        ...
        <!--message below is displayed when user has no open bids-->
        <p id="message" display="none">You have no open bids to display at this time.</p>
        ...
        <table class="table-format" id="table">
          ...
          ?>  
              <tr>
                ...
                <td>
                  <select id="<?php echo $rows['bid_id']; ?>" onchange="withdrawBid(this.id)">
                            <option></option>
          ...
          <script>
           /*Function below will run when the page loads.  If the provider has no open bids, the function will hide the bid table and display a message.*/
           $(document).ready(function(){
                //Set JS variable to number of rows (bids) returned by the sql query at the top of the page
                var bidCount = <?php echo $row ?>;  
                
                //If bid count is zero, do not display the table. Instead, display a message.
                if(bidCount == 0){
          ...
          function withdrawBid(bidID){
              ...
              $.ajax({
                      type: "POST",
                      url: 'withdrawBid.php',  //where the request will be sent
